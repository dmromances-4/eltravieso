...
import { syncHoldedCatalog } from "@/lib/integrations/holded-catalog";
...
export async function POST() {
  ...
  try {
    ...
    const result = await syncHoldedCatalog(profile.id);
    return NextResponse.json({
      message: `Sincronizados ${result.synced} productos desde Holded.`,
      ...result,
    });
  } catch (error) {
