...
export async function generateMetadata({
  ...
  const cocktail = await getRecipeBySlug(params.slug, params.locale as "es" | "en");
  if (!cocktail) return { title: "Cocktail not found" };
  return {
    title: `${cocktail.title} | El Travieso`,
    description: `${cocktail.title} — ${cocktail.kcal} kcal, ${cocktail.abv} ABV`,
    alternates: {
      languages: {
        es: `/recetas/${params.slug}`,
        en: `/en/recetas/${params.slug}`,
      },
    },
  };
}
...
export default async function CocktailPage({
  ...
  const relatedRecipes = await getRelatedRecipes(params.slug, 3, params.locale as "es" | "en");
