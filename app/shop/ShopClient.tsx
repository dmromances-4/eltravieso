...
export default function ShopClient({ products, alcohols }: ShopClientProps) {
  ...
  const chipClass = (active: boolean) =>
    cn(
      ...
      active
        ? 'bg-electric-yellow text-black'
        : 'border border-white/15 text-slate-300 hover:border-electric-blue/40',
    );
  ...
  return (
    <div className="section-shell space-y-12">
      <PageHero
        eyebrow="Tienda"
        title="Llévate la actitud a casa"
        lead="Packs de degustación, botellas individuales, siropes, sodas, material de bar y nuestra enciclopedia líquida."
      />
      ...
      {activeTab !== 'ENCICLOPEDIA' ? (
        <div className="space-y-4 rounded-card border border-white/10 bg-[#111111]/90 p-5">
      ...
      {activeTab === 'ENCICLOPEDIA' ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <EditorialCard
              ...
              footer={
                <div className="space-y-3">
                  ...
                  <Link href={`/shop/${product.slug}`} className="text-sm font-medium text-electric-blue hover:text-white">
                    Ver producto →
                  </Link>
                </div>
              }
          ...
          {filteredProducts.length === 0 ? (
            <p className="col-span-full py-12 text-center text-slate-400">No hay productos en esta categoría por el momento.</p>
          ) : null}
        </div>
      )}
    </div>
